...
if (isset($_POST['Insert'])) {
	for ($x = 0; $x < $counter; $x++) {
		...
		if (isset($_POST['checkbox' . $x])) {
			...
			$SQL = sprintf(
				"INSERT INTO compounds (ReferenceMaterialID, Compound, TargetConcentration, TargetStandardDeviation, TargetBias,IncludeInReport) VALUES (%s,'%s', %s, %s, %s, %s)",
...
<body>
	<div class="box">
		<a href="<?php echo $_SERVER['HTTP_REFERER']; ?>">[return to previous
			page]</a><?php if (isset($_GET['EID']) && isset($_GET['ReferenceMaterialID']) && $counter > 0) { ?>
