...
if (isset($_POST['Submit'])) {
	if (
		$_POST['EID'] == "" || $_POST['ReferenceMaterial'] == "" || $_POST['Compound'] == ""
...
if (isset($_POST['Submit']) && $incomplete == false) {
	...
	if ($checkCompound == 0) {
		$sqlCompoundInsert = sprintf(
			"insert into compounds (referencematerialid,compound,includeinreport,targetconcentration,targetstandarddeviation,targetbias)
									values (%s,'%s',1,%s,%s,%s)",
			$ReferenceMaterialNo,
			$compound,
	...
	for ($i = 1; $i < 12; $i++) {
		...
		if (
			$_POST['secondReplicate' . $i] == "" || $_POST['secondReplicate' . $i] == 0
			...
			$existResult1 = sprintf(
				"delete from results where machine='%s' and compound='%s' and sample='%s' 
			...
			$sqlFirstReplicateResult = sprintf(
				"insert into results (machine,compound,concentration,acqon,sample,top,bottom)
			...
			$sqlSecondReplicateResult = sprintf(
				"insert into results (machine,compound,concentration,acqon,sample)
	...
	if ($db->HasFailedTrans() == false) {
		echo "<h3>MATRIX HAS BEEN SUCCESSFULLY ADDED</h3>";
	}
	...
	<div class="box">
		...
		<form name="form1" method="post" action="">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				...
				<tr>
					<td class="tableheader">
						<div align="left">Target Bias % for Analyte : </div>
					</td>
						...
						<td><select name="firstday<?php echo $x ?>" id="select15">
							...
							<select name="firstyear<?php echo $x ?>" id="select17">
								<?php for ($i = 2002; $i < 2020; $i++) { ?>
									<option value="<?php echo $i ?>" <?php if ($i == 2005) {
																											echo "selected";
																										} ?>> <?php echo $i ?> </option>
								<?php } ?>
							</select>
