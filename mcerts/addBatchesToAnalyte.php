...
$query_Replicates1 = sprintf(
	"select Concentration,AcqOn from results
...
if (isset($_POST['Submit'])) {
	...
	foreach ($existingBatches as $row_ExistingBatches) {
		...
		if (false) //$_POST['secondReplicate'.$i]==""	||$_POST['firstReplicate'.$i]==""	|| $replicate1==$replicate2)
		{
			...
			$deleteExistingBlank = sprintf(
				"delete from results where machine=%s and compound='%s' 
											and sample=concat('%s','BLANK') 
											and to_char(acqon,'YYYY-MM-DD HH24:MI:SS')='%s'",
				$machine,
				$compound,
				$refMat,
				$row_ExistingBatches['ADJUSTMENTTIME']
			);
			...
			if ($blankValue != 0 && $blankValue != "") {
				...
				$insertBlankResult = sprintf(
					"insert into results (concentration,sample,compound,machine,acqon)
										values (%s,concat('%s','BLANK'),'%s',%s,to_date('%s','YYYY-MM-DD HH24:MI:SS'))",
					$blankValue,
					$refMat,
					$compound,
					$machine,
					$row_ExistingBatches['REPLICATE1']
				);
