...
if ($update == false && (isset($_POST["submitted"]))) {
  ...
  $insertSQL = sprintf(
    "INSERT INTO validation_reports (EquipmentID, DateOfApproval, ApprovedBy, Expired,Name,manuallyadded) VALUES (%s,%s, %s, %s, %s,%s)",
...
if ($update == true && (isset($_POST["submitted"]))) {
  $updateSQL = sprintf(
    "UPDATE validation_reports SET DateOfApproval=%s, ApprovedBy=%s, Expired=%s, Name=%s, Locked=%s WHERE EID=%s",
    GetSQLValueString($approvalDate, "date"),
    GetSQLValueString($_POST['ApprovedBy'], "text"),
    GetSQLValueString(isset($_POST['Expired']) ? "true" : "", "defined", "1", "0"),
    GetSQLValueString($_POST['Name'], "text"),
  ...
  if ($update == true) {
    ...
    $approvalDate = $startDate = $endDate = $months = array("", "", "");
  }
...
<body>
    ...
    <p>Equipment ID :
      <select name="EID" id="EID">
        ...
        <?php foreach ($machines as $row_Machines) {    ?>
          <option value="<?php echo $row_Machines['MACHINE'] ?>" <?php if (isset($_GET['EID'])) if (!(strcmp($row_Machines['MACHINE'], $_GET['EID']))) {
                                                                    echo "SELECTED";
                                                                  } ?>><?php echo $row_Machines['MACHINE'] ?></option>
        <?php  } //foreach 
        ?>
      </select>
    </p>
  ...
  <table border="0" cellpadding="0" cellspacing="0" class="InputForm">
    ...
    <tr>
      ...
      <td><input name="Name" type="text" id="Name" value="<?php if (isset($row_ValidationReport['NAME'])) {
                                                            echo $row_ValidationReport['NAME'];
                                                          } ?>" size="40"></td>
    </tr>
    <tr>
      ...
      <td><select name="Aday" id="Aday">
        ...
        <select name="Amonth" id="Amonth">
          <?php for ($counter = 1; $counter < 13; $counter++) { ?>
            <option value=<?php echo "\"" . $counter . "\"";
                          if ($counter == $approvalDate[1]) {
                            echo " selected";
                          } ?>><?php echo $months[$counter - 1] ?></option>
          <?php } ?>
        </select>
        <select name="Ayear" id="Ayear">
          <?php for ($counter = 2000; $counter < 2030; $counter++) { ?>
            <option value=<?php echo "\"" . $counter . "\"";
                          if ($counter == $approvalDate[2]) {
                            echo " selected";
                          } ?>><?php echo $counter ?></option>
          <?php } ?>
        </select>
    ...
    <tr>
      <td>Approved By :</td>
      <td><input name="ApprovedBy" type="text" id="ApprovedBy" value="<?php if (isset($row_ValidationReport['APPROVEDBY'])) {
                                                                        echo $row_ValidationReport['APPROVEDBY'];
                                                                      } ?>"></td>
    </tr>
    <tr>
      <td>Expired : </td>
      <td><input <?php if ($update == true) {
                    if (!(strcmp($row_ValidationReport['EXPIRED'], 1))) {
                      echo "checked";
                    }
                  } ?> name="Expired" type="checkbox" id="Expired" value="checkbox"></td>
    </tr>
    <tr>
      <td>Locked : </td>
      <td><input <?php if ($update == true) {
                    if (!(strcmp($row_ValidationReport['LOCKED'], 1))) {
                      echo "checked";
                    }
                  } ?> name="Locked" type="checkbox" id="Locked" value="true"></td>
    </tr>
  </table>
  <p>
