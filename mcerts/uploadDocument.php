...
if(isset($_POST['submit']) && $_POST['submit']=="true")
{
	...
	if(uploadFile($uploadFrom,$uploadTo))
	{
		if($type=="vf")
		{
			$sql=sprintf("update validation_reports set validationform='%s' where EID=%s",
		...
		{
			$sql=sprintf("update validation_reports set review='%s' where EID=%s",
				$_FILES['userfile']['name'],
				$_POST['EID']);
			$header="index.php";
		}
		...
		{
			$sql=sprintf("update reference_materials set certificatefilename='%s' where referencematerialid=%s",
				$_FILES['userfile']['name'],
...
<input name="submit" type="hidden" value="true">
  ...
  <input type="submit" value="Upload File"></h4>				
