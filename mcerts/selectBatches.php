...
if ((isset($_POST["DeleteAdjustment"])) && ($_POST["DeleteAdjustment"] == "DeleteAdjustment")) 
{
	...
	$sqlUpdateReferenceMaterial=sprintf("update reference_materials set adjustmentmaterial='' where referencematerialid=%s"
									,$refMatId);
...
{
	if($_POST['firstReplicate']>"" && $_POST['secondReplicate']>"" 
		&& $_POST['firstReplicate']!=$_POST['secondReplicate'])  //replicate fields are not empty or equal to each other
	{
		if ($_POST['multiplier']=="") 
			$multiplier=1;
		else
		...
		$insertSQL = sprintf("INSERT INTO batches (ReferenceMaterialID, Replicate1, Replicate2,adjustmenttime,selected,multiplier) VALUES (%s, %s, %s,%s,1,%s)",
...
$sqlAdjustmentReferenceMaterials=sprintf("select distinct sample from results where machine='%s' and acqon between add_months(%s, -1) and add_months(%s,1)",$equipmentID,
 GetSQLValueString($startD, "date"), GetSQLValueString($endD, "date"));
	  ...
	  foreach($existingBatches as $row_ExistingBatches) { ?>
			<?php if (count($existingBatches) > 0) { // Show if recordset not empty ?>
				<tr>
				  ...
				  <input 
				  	<?php if($row_ExistingBatches['SELECTED']=="1") 
							echo "checked";?> type="checkbox" name="checkbox<?php echo $counter ?>" value="checkbox"></td>
