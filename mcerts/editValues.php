...
$updateGoTo = "QC.php?ReferenceMaterialID=".$refMatId."&Compound=".urlencode($compound);

$query_Replicates1 = sprintf("select distinct Concentration,AcqOn from results
	...
	and machine=%s
	                                                 ORDER BY AcqOn"
								  ,$compoundID,$refMatId,$refMatId, $EID);
...
$query_Replicates2 = sprintf("select distinct Concentration, AcqOn from results
							where compound= (SELECT Compound
			                				from compounds
                							where CompoundID=%s)
    						and Sample=(select ReferenceMaterial
...
{
	global $db,$EID;
	$getResultId=sprintf("select resultid, concentration from results
							  ...
							  ,$date
							  ,$sample
							  ,str_replace("'", "''", $compound), $EID);
	$resultRow=$db->GetRow($getResultId);
	$resultId=$resultRow['RESULTID'];
	$concentration=$resultRow['CONCENTRATION'];
	
	$addTrace=sprintf("insert into results_trail (resultid,concentration,username,datechanged,acqon,reason) VALUES (%s,'%s','%s',%s,'%s','%s')",
								$resultId,
								$concentration,
								$_SERVER["REMOTE_USER"],
								$db->DBTimeStamp(time()),
...
if(isset($_POST['Submit']))
{
	for($x=0;$x<count($aConc);$x++)
	{
		...
		if($aConcNew!= $aConc[$x]) //if replicate1 concentration has been changed
		{
			...
			$updateSQL=sprintf("update results set concentration=%s
						where to_char(AcqOn,'YYYY-MM-DD HH24:MI:SS')='%s'
							  and sample='%s'
...
<html>
<head>
<title>Edit Concentrations</title>
...
<h1>Edit Concentrations</h1>

<p><a href="selectBatches.php?ReferenceMaterialID=<?php echo $refMatId?>">[select batches]</a> <a href="ResultsTrace.php?ReferenceMaterialID=<?php echo $refMatId?>&Compound=<?php echo urlencode($compound) ?>">[view audit trail for these results]</a> <a href="QC.php?ReferenceMaterialID=<?php echo $refMatId?>&Compound=<?php echo urlencode($compound) ?>">[view compound report] </a></p>
<table width="62%"  border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    ...
    <td><?php $result=getAssessment($refMatId,$compound); echo $result['precisiontest']?></td>
  </tr>
  <tr>
