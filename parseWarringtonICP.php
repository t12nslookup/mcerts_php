	...
	foreach ($lines as $fullLine)
	{
		...
		if(trim($line[0])=="Average")
		{
			...
			foreach($elements as $element)
			{
				...
				$SQLDelete=sprintf("delete from results where acqon=%s and sample='%s' and compound='%s' and machine=%s",
							$date,
							$sample,
							$elementName,
							$EID);
							
				$SQL=sprintf("insert into results (machine,acqon,concentration,sample,compound, DF,operator,quantmethod,top,bottom,flag)
