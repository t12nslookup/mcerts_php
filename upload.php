...
function parse($fileName)
{
	   ...
	   {  //check if file is a quantitation report
               ...
               $line=preg_split("/,/",$lines[$i]);
				...
				else if(count($line)>3)
				{
				   ...
				   $acqOn=$date;
					...
					array_push($insertSQL,sprintf("delete from results where acqon=%s and compound=%s and machine=%s",
											getSQLDate($date),
											GetSQLValueString($element, "text"),
											$machine)
											);
											
				   array_push($insertSQL,sprintf("INSERT INTO results (DataPath, DataFile,AcqOn, Operator,Sample,SampleMultiplier,Compound,QIon,Response, Concentration, top, bottom,Machine,Flag,Type,SD,RSD,IntSD,IntRSD,ActWgt,ActVol,DF,Sampleno,jobno,sampleconcentration,sampletype) VALUES (%s,%s, %s, %s,%s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s,%s, %s, %s, %s, %s, %s)",
...
function parseQuantitationReport($fileContents)
{
       ...
       for ($i = 0; $i < count($matches[0]); $i++) {
				...
				for ($s = 0; $s< count($samples); $s++) {
					$deleteRow =sprintf("delete from results where acqon=%s and sample='%s' and machine=%s and compound='%s'",
									getSQLDate($acqOn),
									$samples[$s],
									$machine,
									$compound								
